...
def crate_proc_macro(name, srcs = None, **kwargs):
    """`rust_proc_macro` for a workspace member, configured from Cargo metadata.

    A proc-macro crate is compiled for the host and loaded by the compiler, so
    it cannot go through `crate_library`: `rust_library` would build it for the
    target and nothing could load it.

    Args:
      name: the target name, matching the crate's directory.
      srcs: sources; defaults to `src/**/*.rs`.
      **kwargs: passed through to `rust_proc_macro`.
    """
    rust_proc_macro(
        name = name,
        ...
        edition = edition(),
        rustc_flags = WORKSPACE_RUSTC_FLAGS,
        # rules_rs folds proc-macro dependencies into `normal`; there is no
        # separate argument for them as there is in rules_rust.
        deps = all_crate_deps(normal = True),
        **kwargs
    )
...
def crate_tests(
    ...
    if mutants:
        ...
        integration = [
            ...
            if src[len("tests/"):-len(".rs")] not in manual
        ]
